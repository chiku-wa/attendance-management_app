class EmployeesController < ApplicationController
  ...
  def list
    # ----- ソート情報を取得する
    # ソートするカラムを取得する。モデルクラスに存在しないプロパティがパラメータに存在する場合は社員コードをキーとする。
    ...
    # ----- 以下の条件で社員情報一覧を取得
    # * ログインしているユーザは除外する
    # * Viewで必要なテーブルを事前に結合しておく(N+1問題を回避するため)
    # * クエリパラメータのソート情報をもとにソートする、なお冗長性のあるカラム(権限、就業状況などの
    #   外部のマスタテーブルの外部キーなど)でソートした場合にも並び順が保証されるように、
    #   order byの2つめのソートキーには社員コード(昇順)を固定で指定する
